...
public class ItemList implements Serializable{
    ...
    public void save() {
        ...
        try (FileWriter out = new FileWriter(sourceName)) {
            for (int i = 0; i < numItems; i++) {
                out.write(iArray[i].toString() + "\n");  // Correctly write each item
            }
            modified = false;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void removeReservation(String itemId, String memberId) {
        Item item = getItemFromID(itemId);
        if (item != null) {
            item.removeReservation(memberId);
        }
    }
}